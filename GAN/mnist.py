...
from GAN.DataSet import getMnistTrain
...
class Discriminator:
    ...
    def train(self, data=torch.zeros(1)):
        ...
        # train_correct = (out == target).sum() / self.batch_size
        # print("损失为" + str(loss))
        # writer.add_scalar("Loss", loss)
        # print("正确率为" + str(train_correct))
        # writer.add_scalar("Acc", train_correct)
...
def test():
    ...
    # lr = 0.001
    # device = "cuda"
    # modelPath = "model2.pkl"
    # G = Generator(device=device, batch_size=batchSize, lr=lr, model_path=modelPath)
    # imgArr = (G.model.forward().cpu()).detach().numpy()[0].reshape(28, 28) * 255
    # print(imgArr)
    # img = Image.fromarray(imgArr)
    # img.show()
    trainData, target = getMnistTrain(dataSet, 10)
    # imgArr = getImgArr(trainData)
    # print(imgArr.shape)
    # plt.figure(figsize=(28 * 2, 28))
    # plt.imshow(imgArr)
    plt.show()

    # print(trainData)

    # D = Discriminator(data_set=dataSet, device=device, batch_size=batchSize, lr=lr, model_path=modelPath)
    # for i in range(150):
    #     D.train()
    #     D.train(data=torch.rand([batchSize, 28 * 28]))
